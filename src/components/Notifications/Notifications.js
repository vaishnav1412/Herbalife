...
const live = () =>{
  ...
  return (
    <div class="max-w-lg mx-auto items-center h-screen ">
      {notification && (
        <div onClick={()=>{navigate("/user/premiumuser")}} class="flex justify-between px-3 py-1 bg-slate-100 items-center gap-1 rounded-lg border border-gray-300 my-3 shadow-xl">
...
{adminLive && (
      <div onClick={()=>{navigate('/user/videocallroom')}}  class="flex justify-between px-3 py-1 bg-slate-100 items-center gap-1 rounded-lg border border-gray-300 my-3  shadow-xl">
        <div class="relative w-16 h-16 rounded-full hover:bg-red-700 bg-gradient-to-r from-purple-400 via-blue-500 to-red-400">
          <div class="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-14 h-14 bg-gray-200 rounded-full border-2 border-white">
            <img
              class="w-full h-full object-cover rounded-full"
