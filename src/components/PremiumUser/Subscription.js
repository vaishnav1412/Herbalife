...
const Subscription = () => {
   ...
   const getData = async()=>{
    try {
      ...
      const response = await axios.post("/api/user/planlist");
...
try {
  dispatch(showLoading())
  const response = await axios.post("/api/user/purchase", formData);
  dispatch(hideLoading())
...
const verifyPayment =async() =>{
  ...
  try {
    dispatch(showLoading())
    const response = await axios.post("/api/user/verifypayment",formData);
    dispatch(hideLoading())
    if (response.data.success) {
      toast.success(response.data.message);
      ...
      <section className="py-6 dark:bg-gray-300 dark:text-gray-100">
        <div className="container p-4 mx-auto sm:p-10">
          <div className="mb-5 space-y-4 text-center">
            <h1 className="text-4xl font-semibold leadi text-zinc-700 from-stone-800">Select Your Magic Plan</h1>
          </div>
          <div className="grid max-w-lg grid-cols-1 gap-6 mx-auto auto-rows-fr lg:grid-cols-4 lg:max-w-full ">


     {plan.map((plan)=>{
            return<div className=" flex flex-col overflow-hidden border-2 rounded-md dark:border-gray-700 transition duration-300 transform hover:-translate-y-1 hover:shadow-lg hover:bg-violet-200">
              <div className="flex flex-col items-center justify-center px-2 py-8 space-y-4 dark:bg-gray-800">
                ...
                {plan.price}€
                  <span className="text-xl dark:text-gray-400">/{plan.duration}mo</span>
                </p>
              </div>
              ...
              <ul className="self-stretch flex-1 space-y-2">
                ...
                <button className="px-8 py-3 mt-6 text-lg font-semibold rounded sm:mt-12 dark:bg-violet-400 dark:text-gray-900 hover:bg-violet-500" onClick={()=>{purchase(plan._id)}}>
                  Buy Plan
                </button>
              </div>
            </div>
